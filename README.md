...
Reproduction code for **HullFT**: a Frank-Wolfe selector + geometry-aware
integerization + gradient-refresh test-time finetuning for language models.
...
to swap the subset / dataset.

## Reproducibility

Every run calls `hullft.reproducibility.set_global_determinism(seed)` at
entry. That sets `random`, `numpy`, `torch.manual_seed`,
`torch.cuda.manual_seed_all`, `cudnn.deterministic=True`,
`torch.use_deterministic_algorithms(True)`, `CUBLAS_WORKSPACE_CONFIG=:16:8`,
and `PYTHONHASHSEED`.
...
2. **Geometry-aware integerization.** Round the convex weights to integer
   counts that minimize `|| target - sum_i (c_i / N) x_i ||^2` via floor +
   greedy + local swap refinement, then repeat each support point by its count
   to land at exactly `N` instances.
...
Baselines: `knn` (top-N by inner product) and `sift` (SIFT selector).
